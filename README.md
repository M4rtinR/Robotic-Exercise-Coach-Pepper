# Squash Coaching Robot

This branch contains code for running the squash coaching system on Pepper. This README is written with the intention to allow people from Heriot-Watt University and the National Robotarium to run the systems as demonstrations. If you are external to this group, please refer to the master branch to get things up and running and start editing the code for your purposes.

Below are detailed instructions for installing and running the demonstration.

The demonstration is the same code as was used for the long-term evaluation of the system with participants so should be fairly robust. However, they are a bit complex to set up and run and the code is a bit hacked together in places. A description of the studies run and the background to the project can be found in the demos folder in Dropbox: https://www.dropbox.com/home/ITT%20Group%20team%20folder/Demos/Squash%20and%20Stroke%20Rehabilitation.

NOTE: The demonstration will only work on the ITT group's Pepper (the one with the "EM1.69" sticker on the back) due to different tablet versions on the other Peppers. They are set up to work with the ITT Pepper router at the moment. If you wish to run the demos through a different network, you will have to update the IP address in config.py and the IP addresses in the other components.
  
## Instructions for Installing and Running (applicable to both squash and rehabilitation demos)
  ### Requirements
  ...
  The demonstration is split into multiple different components which communicate through API's:
  ...
  You should now be able to select the "long-term squash" branch in PyCharm.
